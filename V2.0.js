...
addBtn.addEventListener('click',()=>{
  ...
  const review={id:Math.random().toString(36).slice(2),park,text,stars,platform,language,date:new Date().toISOString(),important,fistbump};
  reviews.push(review);
  rerenderList();
  ...
  document.getElementById('language').selectedIndex=0; document.getElementById('important').checked=false; document.getElementById('fistbump').checked=false;
});
...
exportBtn.addEventListener('click',()=>{
  ...
  const clean=reviews.map(r=>({
    ...
    important:r.important?'X':'',fistbump:r.fistbump?'X':'',date:r.date
  }));
