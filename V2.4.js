...
if (fistbumpCheckbox && fistbumpInput) {
  fistbumpCheckbox.addEventListener("change", e => {
    if (e.target.checked) {
      ...
      setTimeout(() => (fistbumpInput.style.opacity = "1"), 10);
    } else {
      fistbumpInput.style.opacity = "0";
      setTimeout(() => (fistbumpInput.style.display = "none"), 200);
      fistbumpInput.value = "";
    }
  });
}
...
function detectLanguage(text) {
  ...
  const count = arr => arr.reduce((a,w)=>a+(lower.includes(w)?1:0),0);
...
function rerenderList(){
  ...
  for(let i=reviews.length-1;i>=0;i--){
    ...
    const platSel=item.querySelector(".platform-select"); platSel.value=r.platform;
    platSel.addEventListener("change",e=>r.platform=e.target.value);
    const langSel=item.querySelector(".language-select"); langSel.value=r.language;
    ...
    item.querySelector(".edit-btn").onclick=()=>{
      ...
      if(newText!==null){
        ...
        if(reply && navigator.clipboard)
          navigator.clipboard.writeText(reply).then(()=>showToast("Review updated; reply copied."));
...
addBtn.addEventListener("click",()=>{
  ...
  if(reply && navigator.clipboard)
    navigator.clipboard.writeText(reply).then(()=>showToast("Review added; reply copied."));
  else showToast("Review added.");

  el.value=""; document.getElementById("stars").value="";
  document.getElementById("platform").selectedIndex=0;
  document.getElementById("language").selectedIndex=0;
  document.getElementById("important").checked=false;
  document.getElementById("fistbump").checked=false;
  if(fistbumpInput){fistbumpInput.value="";fistbumpInput.style.opacity="0";setTimeout(()=>fistbumpInput.style.display="none",200);}
});
